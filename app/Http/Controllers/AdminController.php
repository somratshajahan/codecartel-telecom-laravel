...
class AdminController extends Controller
{
    ...
    public function confirmRegularRequest(Request $request, $id)
    {
        ...
        $regularRequest->update(['status' => 'approved']);

        \DB::table('recharge_history')->insert([
            'user_id' => $regularRequest->user_id,
            'amount' => $regularRequest->amount,
            'type' => 'Internet Pack - ' . $regularRequest->operator,
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    ...
    public function allHistory()
    {
        ...
        $regularRechargeHistory = \DB::table('recharge_history')
            ...
            ->select('recharge_history.*', 'users.name as user_name')
            ...
            ->map(function ($item) {
                return (object) [
                    ...
                    'operator' => $item->type ?? 'Internet Pack',
                    'mobile' => '-',
                    'amount' => $item->amount,
                    'status' => 'success',
                    'description' => 'Internet Pack Recharge',
        ...
        $operatorSales = collect();
        $bankingSales = collect();
        return view('admin', compact('settings', 'operators', 'allHistory', 'pendingCount', 'totalAmount', 'totalUsers', 'today', 'yesterday', 'balanceToday', 'balanceYesterday', 'operatorSales', 'bankingSales'));
    }
}